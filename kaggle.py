"""
F1 进站预测 —— CatBoost 主模型 + 残差 MLP + Optuna 调优
"""
import warnings
import os
...
from sklearn.preprocessing import LabelEncoder          # 将类别特征编码为整数，供神经网络使用
...
print(f"训练集: {train_raw.shape}  测试集: {test_raw.shape}  正样本比例: {train_raw[TARGET].mean():.4f}")

y = train_raw[TARGET].values.astype(int)
POS_W = (y == 0).sum() / max((y == 1).sum(), 1)   # 类别权重，用于应对不平衡

# ===================== 特征工程 =====================
def features(df):
    """从单行数据中构造大量衍生特征，涵盖多项式、比率、分段标记等"""
    df = df.copy()
    for c in CAT_COLS:
    ...
    # 赛道阶段标记
    df["IsEarly"]  = (df["RaceProgress"] < 0.25).astype(int)
    df["IsMid"]    = ((df["RaceProgress"] >= 0.25) & (df["RaceProgress"] < 0.75)).astype(int)
    df["IsLate"]   = (df["RaceProgress"] >= 0.75).astype(int)
    df["IsLead"]   = (df["Position"] == 1).astype(int)
    df["IsTop5"]   = (df["Position"] <= 5).astype(int)
    df["IsStint1"] = (df["Stint"] == 1).astype(int)
    df["IsPit0"]   = (df["PitStop"] == 0).astype(int)
...
def add_relative_features(tr_df, te_df):
    """利用全量数据（训练+测试）构建相对特征，比如同圈对比、相邻车辆信息等（Part B）"""
    eps = 1e-6
    n_tr = len(tr_df)
    ...
    both["rel_same_comp_ahead"]  = (both["Compound"] == cp_ahead).astype(int)
    both["rel_same_comp_behind"] = (both["Compound"] == cp_behind).astype(int)
    both["rel_undercut_threat"]  = ((tl_behind < both["TyreLife"]) & (both["rel_pace_vs_behind"] > 0)).astype(int)
    both["rel_overcut_setup"]    = ((tl_ahead  > both["TyreLife"]) & (both["rel_pace_vs_ahead"] <= 0)).astype(int)
    ...
    both["rel_is_final_laps"]   = (both["rel_laps_remaining"] <= 5).astype(int)
    both["rel_can_amortize"]    = (both["rel_laps_remaining"] >= 8).astype(int)
...
if USE_NN:
    # 记录每个类别特征的词表大小（+1 留出余量，防止越界）
    cat_cards = {}
    for c in CAT_COLS:
        cat_cards[c] = int(X_train_mlp[c].max()) + 1
...
# ===================== 多 seed 交叉验证，产生 OOF 和测试集预测 =====================

def train_catboost_cv(params, seeds):
    """训练 CatBoost，返回 OOF 预测、测试集预测及 AUC 统计"""
    oof = np.zeros(len(y), dtype=np.float64)
    oof_count = np.zeros(len(y), dtype=np.int32)   # 记录每个样本被预测了几次
...
submission[TARGET] = final_test      # 直接赋值，避免 merge 可能带来的 NaN
